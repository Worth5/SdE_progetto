 ...
 * - attende un numero intero n dal client
 * - attenda una stringa di lunghezza n dal client
 * - chiude il socket
 *
 */
...
int main(int argc, char* argv[])
{
    ...
    rcvd_bytes = recv(conn_sd, buff, n, 0);
    if (rcvd_bytes < 0)
    {
        fprintf(stderr, "Impossibile ricevere dati su socket: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    ...
    // --- INVIO RISPOSTA --- //
    size_t snd_bytes = send(conn_sd, "OK", 2, 0);
    if (snd_bytes < 0)
    {
